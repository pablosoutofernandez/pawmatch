            ...
            <div class="w-full flex flex-wrap justify-between items-center px-4 sm:px-6 lg:px-8 py-3">
                ...
                <div class="flex items-center gap-2 sm:gap-4">
                    <form method="GET" action="{{ route('perfil') }}">
                        <button type="submit" class="text-slate-600 hover:text-brand-600 font-bold py-2 px-2 sm:px-4 transition-all flex items-center gap-2 text-sm">
                            Hola {{ Auth::user()->name }}
                        </button>
                    </form>
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="text-slate-600 hover:text-red-600 font-bold py-2 px-2 sm:px-4 transition-all flex items-center gap-2 text-sm">
